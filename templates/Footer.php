...
<!-- new -->
<?php if($parameters[0]=="results"){ ?>
    <script type="text/javascript" src="<?php echo Request::$BASE_URL; ?>vendor/arbor/lib/arbor.js"></script>
    <script type="text/javascript" src="<?php echo Request::$BASE_URL; ?>vendor/arbor/lib/arbor-tween.js"></script>
    <script type="text/javascript" src="<?php echo Request::$BASE_URL; ?>vendor/arbor/lib/arbor-graphics.js"></script>
            ...
            var Renderer = function(elt){
                ...
                var that = {
                    ...
                    resize:function(){
                        ...
                        canvas.width = $("#rightPanel").width()*0.9
                        canvas.height = "400"
                        sys.screen({size:{width:canvas.width, height:canvas.height}})
                        _vignette = null
                        that.redraw()
                    ...
                    update:function(){
                        ...
                        switch (_path){
                            ...
                            case 'reference':
                                ...
                                document.title = "arbor.js ﾃつｻ " + _path
                                break
                        }
            ...
            $(document).ready(function(){
                ...
                var theUI = {
                    nodes:{
                        ...
                        "<?php echo $prophetic_food->food ?>":{color:"red", shape:"dot", alpha:1}

                        <?php } ?>

                        <?php if(isset($pfs) && count($pfs)>0){  ?>
                        ,

                        <?php $num=count($pfs); $x=0; foreach($pfs as $p){ $x++; ?>
                        <?php if($p['name']!=$prophetic_food->food){ ?>
                        "<?php echo $p['name'] ?>":{color:CLR.branch, shape:"dot", alpha:1}<?php if($num!=$x){echo ",";} ?>
                        <?php } ?>
                        <?php } ?>
                    ...
                    edges:{
                        <?php if(isset($prophetic_food) && $prophetic_food->food!=""){ ?>
                        "<?php echo $prophetic_food->food ?>":{
                            <?php $num=count($pfs); $x=0; foreach($pfs as $p){ $x++; ?>
                            <?php if($p['name']!=$prophetic_food->food){ ?>
                            "<?php echo $p['name'] ?>":{length:.8}<?php if($num!=$x){echo ",";} ?>
                            <?php } ?>
                            <?php } ?>
                        }
    ...
    <script type="text/javascript">
        ...
        function showQuran(){
            ...
            $("#quranLink").addClass('active');
            $("#allLink").removeClass('active');
            $("#hadithLink").removeClass('active');
            $("#articleLink").removeClass('active');
            $("#manuLink").removeClass('active');
        }
        ...
        function showHadith(){
            ...
            $("#hadithLink").addClass('active');
            $("#allLink").removeClass('active');
            $("#articleLink").removeClass('active');
            $("#quranLink").removeClass('active');
            $("#manuLink").removeClass('active');
        }
